...
const HIDEOUT_DOOR = { x: 480, y: 480 };
const HIDEOUT_DETECT = 50;
const TOWN_HOUSE = { x: 320, y: 360 };
const TOWN_DOOR = { x: 320, y: 396 };
const DOOR_DETECT = 40;

export default class SuperRetroMapScene extends Phaser.Scene {
  constructor() { super('SuperRetroMap'); }
  ...
  create(data) {
    ...
    initMp(this, { returnScene: 'SuperRetroMap', spriteScale: 1.5, tagOffset: -18, username: this._username });
    ...
    if (data?.ws && data.ws.readyState === WebSocket.OPEN) {
      ...
      const px = Math.round(data.playerX || this.spawnX);
      const py = Math.round(data.playerY || this.spawnY);
      joinRoom(this, 'retro', px, py);
    } else {
    ...
    const ox = TOWN_HOUSE.x - 16, oy = TOWN_HOUSE.y - 16;
    if (this.textures.exists('retro_house_tl')) this.add.image(ox + 8, oy + 8, 'retro_house_tl').setDepth(4);
    if (this.textures.exists('retro_house_tr')) this.add.image(ox + 24, oy + 8, 'retro_house_tr').setDepth(4);
    if (this.textures.exists('retro_house_bl')) this.add.image(ox + 8, oy + 24, 'retro_house_bl').setDepth(4);
    if (this.textures.exists('retro_house_br')) this.add.image(ox + 24, oy + 24, 'retro_house_br').setDepth(4);
    if (this.textures.exists('retro_door')) {
      this.add.image(TOWN_HOUSE.x, TOWN_HOUSE.y + 16, 'retro_door', 0).setDepth(5);
    }
    this.add.text(TOWN_HOUSE.x, TOWN_HOUSE.y - 32, 'TOWN PORTAL', {
      ...FONT, fontSize: '6px', fontStyle: 'bold',
  ...
  buildMap() {
    const map = this.make.tilemap({ key: 'super-retro-map' });
    const tileset = map.addTilesetImage('gigantic_pack', 'super-retro-tiles');
    ...
    this.spawnX = spawnObj ? spawnObj.x : 640;
    this.spawnY = spawnObj ? spawnObj.y : 800;
  }
  ...
  drawHud() {
    this.add.text(8, 4, 'SUPER RETRO', {
      ...FONT, fontSize: '14px', color: '#ff8844', stroke: '#000', strokeThickness: 3
  ...
  update(time) {
    ...
    if (!cs) {
      const townDoorDist = Phaser.Math.Distance.Between(this.player.x, this.player.y, TOWN_DOOR.x, TOWN_DOOR.y);
      const hideoutDist = Phaser.Math.Distance.Between(this.player.x, this.player.y, HIDEOUT_DOOR.x, HIDEOUT_DOOR.y);
      if (townDoorDist < DOOR_DETECT) {
        this.promptText.setText("[E] Enter Town");
        if (ePressed) { this.enterTownHouse(); return; }
      } else if (hideoutDist < HIDEOUT_DETECT) {
  ...
  _toggleChat() {
    ...
    headerTitle.style.color = '#44ffaa'; headerTitle.style.fontSize = '11px';
    headerTitle.textContent = 'CHAT & PLAYERS';
